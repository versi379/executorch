...
TEST_F(OpGridSampler2DOutKernelTest, BasicFunctionality) {
  ...
  Tensor input = TensorFactory::from_data<float>({1, 3, 4, 4}, {
  ...
  Tensor grid = TensorFactory::from_data<float>({1, 4, 4, 2}, {
  ...
  Tensor out = TensorFactory::create<float>({1, 3, 4, 4}); // Same shape as input
